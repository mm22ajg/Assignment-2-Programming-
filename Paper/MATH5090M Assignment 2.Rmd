...
Now we can implement the BOP2 clinical trial design in R. Here we want to minimise the expected sample size subject to the type I error rate being at most $0.05$ and the type II error rate being at most $0.2$. For the prior distribution we will use $a=0.5$ and $b=0.5$ here. We need to program two functions which are capable of calculating the expected sample size and the error rates.
...
The minimum expected sample size found by the algorithm is 27.657. This is achieved when $\lambda=0.95$, $\gamma=0.9$, $n_1=8$, and $n_2=40$.
